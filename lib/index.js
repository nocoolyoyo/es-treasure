...
function getFileType(fileInfo) {
  ...
  * */
	var bitWorkFlow = function bitWorkFlow(bitHead) {};
  ...
  * @return fileType {String}
  * */
	var nameWorkFlow = function nameWorkFlow(fileExt) {};

	/*主程
  * */
	function run() {
		fileInfo = fileInfo.toLowerCase();
		var ext = /\./.test(fileInfo) === true ? fileInfo.split["."][1] : fileInfo;

		for (var k in nameMatchGroup) {
